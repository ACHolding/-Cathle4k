...
"""
cathle4k0.1a.py — cat64hle 1.x N64 HLE emulator shell (clean-room Python monolith)
Engine: cathle1.x
...
APP_NAME = "cat64hle"
VERSION = "4k0.1a-python314-vi-interrupt"
BUILD_TAG = "cathle4k0.1a-py314-cathle1x-vi-interrupt"
...
# VI interrupt glue for cathle4k0.1a.
# The R4300i sees N64 MI/VI events on CP0 Cause.IP2. Keeping ExcCode at 0
# makes the handler see a real interrupt instead of a syscall-like exception.
VI_INTERRUPT_VECTOR = 0x80000180
VI_INTERRUPT_INTERVAL = FRAME_STEPS_PER_TICK
CP0_STATUS_IE = 0x00000001
CP0_STATUS_EXL = 0x00000002
CP0_STATUS_ERL = 0x00000004
CP0_STATUS_INTERRUPT_MASK = 0x0000FF00
CP0_CAUSE_IP2 = 0x00000400
CP0_CAUSE_EXCCODE_MASK = 0x0000007C
MI_INTR_REG = 0x04300008
MI_INTR_MASK_REG = 0x0430000C
MI_INTR_VI = 0x00000008
VI_CURRENT_REG = 0x04400010
...
class DeviceBus:
    ...
    def reset(self):
        ...
        self.regs[MI_INTR_REG] = 0
        self.regs[MI_INTR_MASK_REG] = 0
        self.regs[VI_ORIGIN_REG] = 0
        self.regs[VI_WIDTH_REG] = 320
        self.regs[VI_CURRENT_REG] = 0
    ...
    def handle_mmio(self, addr: int, val: int):
        if addr == MI_INTR_MASK_REG:
            mask = self.regs.get(MI_INTR_MASK_REG, 0)
            # N64 MI writes use clear/set pairs for each interrupt source.
            pairs = (
                (0x0001, 0x0002, 0x01),  # SP
                (0x0004, 0x0008, 0x02),  # SI
                (0x0010, 0x0020, 0x04),  # AI
                (0x0040, 0x0080, 0x08),  # VI
                (0x0100, 0x0200, 0x10),  # PI
                (0x0400, 0x0800, 0x20),  # DP
            )
            for clear_bit, set_bit, source_bit in pairs:
                if val & clear_bit:
                    mask &= ~source_bit
                if val & set_bit:
                    mask |= source_bit
            self.regs[MI_INTR_MASK_REG] = mask & 0x3F
        elif addr == VI_CURRENT_REG:
            # Games clear the VI interrupt by writing VI_CURRENT/VI_V_CURRENT.
            self.regs[MI_INTR_REG] = self.regs.get(MI_INTR_REG, 0) & ~MI_INTR_VI
            self.core.cpu.cp0[CP0_CAUSE] &= ~CP0_CAUSE_IP2
        elif addr == 0x0460000C:  # PI DMA Write
...
class CPUCore:
    ...
    def step(self):
        ...
        if word == 0:
            ...
            # Simulate VI interrupt (60 times per second) for idle/wait loops too.
            self._simulate_vi_interrupt()
            self.gpr[0] = 0
    ...
    def _mi_vi_mask_allows_interrupt(self) -> bool:
        # Keep this compatibility-friendly: if a ROM never touched MI_MASK,
        # still allow the CP0-based prompt fix to wake its libultra loop.
        mi_mask = self.core.bus.regs.get(MI_INTR_MASK_REG, 0)
        return mi_mask == 0 or bool(mi_mask & MI_INTR_VI)

    def _simulate_vi_interrupt(self) -> bool:
        """Tick a 60 Hz VI interrupt source and vector to libultra's handler.

        This is the requested markdown fix turned into real cathle1.x code. The
        visible change is that games waiting for VI/vblank now get periodic CPU
        interrupts at 0x80000180 instead of sitting forever on a static frame.
        """
        self.core.vi_counter += 1
        if self.core.vi_counter < VI_INTERRUPT_INTERVAL:
            return False

        self.core.vi_counter = 0
        self.core.bus.regs[MI_INTR_REG] = self.core.bus.regs.get(MI_INTR_REG, 0) | MI_INTR_VI

        status = self.cp0[CP0_STATUS]
        cause = self.cp0[CP0_CAUSE]
        status_allows_line = bool(status & CP0_STATUS_INTERRUPT_MASK)
        already_pending = bool(cause & CP0_CAUSE_IP2)
        if not status_allows_line or already_pending or not self._mi_vi_mask_allows_interrupt():
            return False

        self.cp0[CP0_CAUSE] = cause | CP0_CAUSE_IP2
        interrupts_enabled = (
            (status & CP0_STATUS_IE) != 0
            and (status & (CP0_STATUS_EXL | CP0_STATUS_ERL)) == 0
        )
        if not interrupts_enabled:
            return False

        self.cp0[CP0_EPC] = self.pc
        self.cp0[CP0_CAUSE] = (self.cp0[CP0_CAUSE] & ~CP0_CAUSE_EXCCODE_MASK) | CP0_CAUSE_IP2
        self.pc = VI_INTERRUPT_VECTOR
        self.next_pc = u32(self.pc + 4)
        return True
    ...
    def execute(self, o: N64Opcode):
        ...
        self.pc = self.next_pc
        self.next_pc = u32(self.next_pc + 4)
        # In execute(), after the PC update logic: simulate VI interrupt
        # roughly once per frame so libultra vblank waits can continue.
        if self._simulate_vi_interrupt():
            return
        g = self.gpr
...
class ACsN64Core:
    ...
    __slots__ = (
        ...
        "frame_count", "hle_calls", "vi_counter", "controller_state", "rdp_draw_commands",
